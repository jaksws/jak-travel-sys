...
<head>
    ...
    <!-- Styles -->
    <link rel="stylesheet" href="{{ asset('css/V1/app.css') }}">
    @stack('styles')

    <!-- Scripts -->
    <script src="{{ asset('js/V1/app.js') }}" defer></script>
    <script>
        window.darkModeSettings = @json(config('V1_features.dark_mode'));
...
<body class="font-sans antialiased min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
    <div id="app">
        <!-- Navigation Header -->
        @include('layouts.V1.navigation')

        <!-- Language Selector -->
        @if(config('V1_features.multilingual.enabled'))
            @include('layouts.V1.language-selector')
        @endif

        <!-- Page Content -->
        <main class="container mx-auto py-4 px-4 sm:px-6 lg:px-8">
            @include('layouts.V1.flash-messages')
            @yield('content')
        </main>
    ...
    @if(config('V1_features.dark_mode.enabled'))
        <button 
            ...
            aria-label="{{ __('V1.toggle_dark_mode') }}"
        >
